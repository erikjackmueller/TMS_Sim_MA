    ...
    for method in methods:
        ...
        for i in range(len(r_ins)):
            ...
            fn_fig = location + "-" + method + "-" + orientation + "-" + value_under_test + \
                     "-" + str(np.round(r_ins[i], 2)) + ".png"
            if not os.path.isfile(fn_fig) or overwrite:
                res1, res, xyz_grid = One_layer_sphere_single_m_test(n=n, r_out=r_out, r_in=r_ins[i],
                ...
                values.append(np.round(r_ins[i], 2))
            ...
            iter_left = (2 * 2 * len(r_ins)) - count
            t_left = t_format(time_needed * iter_left)
            print(fn_fig[:-4])
            print(f"iteration {count}/{2 * 2 * len(r_ins)} done!")
