...
for orientation in orientations:
    ...
    for method in methods:
        ...
        for i in range(len(fs)):
            ...
            iter_left = (2 * 2 * len(fs)) - count
            t_left = t_format(time_needed * iter_left)
            print(fn_fig[:-4])
            print(f"iteration {count}/{2 * 2 * len(fs)} done!")
