...
tc, areas, tri_points = sphere_mesh(500, scaling=scaling_factor)
end = time.time()
t = t_format(end - start)
...
start = time.time()
Q, rs, A, b = SCSM_tri_sphere_numba(tc, tri_points, areas, r0=r0, m=m)
end = time.time()
t = t_format(end - start)
print(f"{t[0]:.2f}" + t[1] + "  Q linalg.solve()")

start = time.time()
# Q, rs = SCSM_tri_sphere(tc, tri_points, areas, r0=r0, m=m, sig=0.33)

Q = SCSM_jacobi_iter_debug(tc, tri_points, areas, r0=r0, m=m, tol=1e-2, initial_guess=True)
end = time.time()
t = t_format(end - start)
